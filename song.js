...
css += "div.hQfrn, .OH0apf { display: none !important; }";	
		...
		if (response.o.contact) {
			css+=".spContact, .spTask{position:absolute;display: block;padding: 0 6px;z-index:1002}";
			css+=".spContact{right:220px}";
			css+=".spTask{right:280px}";
		}
		...
		var html = 	'<span class="gbts spContact">Contacts</span>'+
					'<span class="gbts spTask">Tasks</span>';
		
		$('#gbg').before(html);
		$('.spContact, .spTask').hover(
			function(e){
				$(this).addClass('gbzt gbzt-hvr');
			},
			function(){
				$(this).removeClass('gbzt gbzt-hvr');
			}
		);
